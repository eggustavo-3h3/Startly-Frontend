...
export class CadastroComponent implements OnInit {
  ...
  adicionarAtuacao() {
    if (this.atuacaoSelecionadaAdicionar) {
      const atuacao = this.atuacoes.find(
        (a) => a.id === this.atuacaoSelecionadaAdicionar,
      );
      if (
        atuacao &&
        !this.atuacoesSelecionadas.some((a) => a.id === atuacao.id)
      ) {
  ...
  salvarStartup() {
    ...
    (dadosStartup as any).atuacoes = this.atuacoesSelecionadas.map((a) => ({
      atuacaoId: a.id,
    }));
